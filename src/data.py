"""CIFAR-10 and CIFAR-10-C data loaders.

CIFAR-10 comes from torchvision (crop + flip for training, normalize only for
eval). The geometry subset is a fixed 2,000-image slice of the training set with
no augmentation, the same images for every model. CIFAR-10-C is loaded from the
.npy files produced by data/download_cifar10c.py.
"""
...
# The 15 test corruptions from Hendrycks & Dietterich (2019). The archive also
# ships 4 validation corruptions (speckle_noise, gaussian_blur, spatter,
# saturate); the pilot does not use them.
...
def build_geometry_loader(
    ...
    """Fixed subset of the CIFAR-10 training set for all geometry measurements.

    subset_seed is a fixed constant, not the run seed, so every model is measured
    on exactly the same images. No shuffling, so the iteration order is stable.
    """
    base = datasets.CIFAR10(str(data_dir), train=True, download=download,
                            transform=eval_transform())
    rng = np.random.default_rng(subset_seed)
    indices = np.sort(rng.choice(len(base), size=n_examples, replace=False))
...
class CIFAR10C(Dataset):
    ...
    Each <corruption>.npy holds 5 severity blocks stacked in order; labels.npy
    is the CIFAR-10 test labels repeated once per block.
    """
    ...
    def __init__(self, root: str | Path, corruption: str, severity: int,
        ...
        if not images_path.exists() or not labels_path.exists():
            raise FileNotFoundError(
                ...
                f"Run: python data/download_cifar10c.py --dest {root}"
            )
        ...
        per = len(images) // 5                  # 10000 for the real dataset
        lo, hi = (severity - 1) * per, severity * per
    ...
    def __getitem__(self, idx: int):
        # ToTensor turns the HWC uint8 array into a CHW float tensor in [0, 1].
...
def iter_cifar10c_loaders(
    ...
    """Yield (corruption, severity, loader) over the whole CIFAR-10-C sweep."""
